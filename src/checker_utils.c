...
/*   Updated: 2025/12/23 10:11:03 by jsouza           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	limit_checker(long *array, int size, t_stack **stack)
{
	...
	while (i < size)
	{
		if (array[i] > INT_MAX || array[i] < INT_MIN)
			return (ft_fprintf(2, "Error limits\n"), free_all(stack),
				exit(EXIT_FAILURE));
		i++;
	}
}
...
void	double_checker(long *array, int size, t_stack **stack)
{
	...
	while (i < size)
	{
		...
		{
			if (array[i] == array[j])
				return (ft_fprintf(2, "Error double\n"), free_all(stack),
					exit(EXIT_FAILURE));
			j++;
		}
		i++;
...
void	first_checker(int ac, char **av)
{
	...
	if (ac <= 1)
		return (ft_fprintf(2, "Error no arg\n"), exit(EXIT_FAILURE));
	i = 1;
	while (i < ac)
	{
		j = 0;
		while (av[i][j])
		{
			if (av[i][j] && ((!ft_isdigit(av[i][j]) && !ft_isspace(av[i][j])
						...
						&& !ft_isdigit(av[i][j + 1]))))
				return (ft_fprintf(2, "Error !numbers %c\n", av[i][j]),
					exit(EXIT_FAILURE));
			j++;
		}
		i++;
